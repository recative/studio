...
import { BundleOptionItem } from 'pages/Bundle/components/BundleOptionItem';
...
export const useDeployBundleModal = ModalManager<number, null>(null);

export const useSelectedProfile = () => {
  const [selectedProfiles, setSelectedProfiles] = React.useState(
    () =>
      new Set(
        (localStorage.getItem('@recative/studio/selectedDeployProfile') ?? '')
          .split(',,,')
          .filter(Boolean)
      )
  );
  ...
  const handleSelectProfile = React.useCallback(
    (checked: boolean, id: string) => {
      ...
      localStorage.setItem(
        '@recative/studio/selectedDeployProfile',
...
export const DeployBundleModal: React.FC = () => {
  ...
  const [showBundleOption, data, , onClose] = useDeployBundleModal();
            ...
            {profiles.result?.map((x) => {
              return (
                <BundleOptionItem
                  key={x.id}
                  title={x.label}
                  description={x.targetUploaderId}
                  id={x.id}
