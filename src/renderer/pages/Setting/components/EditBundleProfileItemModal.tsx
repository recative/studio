  ...
  const availableProfileExtensions = React.useMemo(() => {
    return (
      extensionList.result?.bundler.map((extension) => ({
        ...
        Icon: extension.iconId ? icons[extension.iconId] : null,
      })) ?? []
    );
...
> = ({ onSubmit }) => {
  ...
  const resourceTagSelectorCallbacks = useEventWrappersForResourceTagSelector(
    ...
    valueChangeCallbacks.excludedResourceTags
  );
    ...
    >
      ...
      {profileId !== null ? (
        <ModalBody>
          ...
          {offlineAvailabilityOptionValue[0]?.id !== 'bare' && (
            <FormControl
              label="Exclude Resource Tags"
              caption="Resource files with these tags will be ignored from bundling."
            >
              <ResourceTagSelector
                value={clonedProfile?.excludedResourceTags}
                size={BUTTON_SIZE.mini}
                {...resourceTagSelectorCallbacks}
              />
            </FormControl>
          )}
          <FormControl
